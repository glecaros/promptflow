...
class Summary:
    ...
    # Parse inputs and outputs from span events and truncate the content
    ...
    def _truncate_inputs_outputs_content(self, content):
        def _process_value(value):
            # Placeholder text for different collection types
            list_placeholder = "[LIST]"
            dict_placeholder = "[DICT]"

            # Check the type of value and process accordingly
            if isinstance(value, (int, float)):
                # Keep numeric values as they are
                return value
            elif isinstance(value, str):
                # Truncate string values
                return value[:100]  # Truncate to first 100 characters
            elif isinstance(value, list):
                # Use placeholder for list
                return list_placeholder
            elif isinstance(value, dict):
                # Use placeholder for dictionary
